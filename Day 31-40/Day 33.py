...
#asking user to enter the music note
note = input("Enter the music note: ").upper().strip()

#splitting the characters of the music note for the extraction of the frequency of other notes
note_apha = note[0]
note_octave = int(note[1])

#making a list of valid notes letters
note_alphabets = ["A", "B", "C", "D", "E", "F", "G"]

#filtering out non-valid entries:
#first is entries with character count that is not 2, since character counts of all valid notes is 2
#second filters entries that contain non-alphabets and non-numeric charcters, e.g. punctuations
#third is for note alphabets that are not between "A" to "G"
#and the last is note octaves above 8
if len(note) != 2 or note.isalnum() == False or note_apha not in note_alphabets or note_octave not in range(0,9):
  print("Please enter a valid music note")

#specifying corresponding frequencies of the fourth octave of  each note
else:
  if note_apha == "C":
    frequency = 261.63
  ...
  #generating the frequencies of other octaves based on given formula
...
#using try/except to filter out non-numerical inputs
try:

  #asking user to enter frequency
  reverse_frequency = float(input("Enter the frequency of the music note: "))

  #defining corresponding notes for each frequency range by adding
  #...and subtracting 1 to/from the given note inthe previous task
...
#error message for non-numerical inputs
except:
